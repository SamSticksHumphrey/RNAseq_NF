...
if (length(args) < 2) {
    stop("Usage: rsem_tximport.r <coldata> <rsem_out>", call.=FALSE)
}
...
rsem_output <- path # list.files(path, pattern = "isoform.results", recursive = T, full.names = T)

# run tx import
txi_gene <- tximport::tximport(rsem_output, type = "rsem", txIn = TRUE, tx2gene = tx2gene, countsFromAbundance = "no")
txi_gene_scaledTPM <- tximport::tximport(rsem_output, type = "rsem", txIn = TRUE, tx2gene = tx2gene, countsFromAbundance = "scaledTPM")
txi_gene_lengthScaledTPM <- tximport::tximport(rsem_output, type = "rsem", txIn = TRUE, tx2gene = tx2gene, countsFromAbundance = "lengthScaledTPM")

txi_tx = tximport::tximport(rsem_output, type = "rsem", txOut = TRUE)
txi_dtuScaledTPM <- tximport::tximport(rsem_output, type = "rsem", txOut = TRUE, tx2gene = tx2gene, countsFromAbundance = "dtuScaledTPM")

# Abundance is the TPM table from the original rsem quantification, but gene-level summarized
...
# The count table from the original rsem quantification, but gene-level summarized.
...
# Gene lengths from the original rsem quantification
...
# The count table from the original rsem quantification at transcript level.
...
# The abundance from the original rsem quantification at transcript level.
...
# The transcript lengths from the original rsem quantification.
